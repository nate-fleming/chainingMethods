...
let profits = cars.map(car => {
    return car.gross_profit
})

let totalProfits = profits.reduce((totalNum, currentNum) => totalNum += currentNum).toFixed(2)
...
cars.forEach(car => {
    sellDates.push(Number(car.purchase_date.slice(5, 7)))
})

sellDates.sort(function (a, b) { return a - b })
// console.log(sellDates)

// let result = {}
// sellDates.forEach(function (v, i) {
//     if (!result[v]) {
//         result[v] = [i]
//     } else {
//         result[v].push(i)
...
cars.forEach(car => {
    salesPeople.push(`${car.sales_agent.first_name} ${car.sales_agent.last_name}`)
})

console.log(salesPeople)


let most,  // This is the value we will return
...
function findMost(array) {
    ...
    //will return most frequent item in the array
    return most;
}


console.log(findMost(salesPeople))

console.log(findMost(sellDates))


// salespeople.foreach get the profits and add them using reduce



const personalProfits = cars.map(car => {
    let person = {
        name: `${car.sales_agent.first_name} ${car.sales_agent.last_name}`,
...
console.log(personalProfits)

var result = _.chain(personalProfits).groupBy('name').map(function (v, i) {
    return {
        name: i,
...
console.log(`The person with the highest sales is ${result[0].name} with $${result[0].profit} in profits`)
